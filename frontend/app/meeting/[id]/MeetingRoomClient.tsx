...
export default function MeetingRoomClient({ id }: { id: string }) {
  const [isVideoOn, setIsVideoOn] = useState(false);
  const [isMicOn, setIsMicOn] = useState(false);
  const [isScreenSharing, setIsScreenSharing] = useState(false);
  const [participants] = useState([{ id: 1, name: 'You', isHost: true, isVideoOn: false, isMicOn: false }]);

  const toggleVideo = () => setIsVideoOn((v) => !v);
  const toggleMic = () => setIsMicOn((v) => !v);
  const toggleScreenShare = () => setIsScreenSharing((v) => !v);
    ...
    <div className="min-h-screen bg-gray-900 text-white">
      <div className="bg-gray-800 p-4 flex items-center justify-between">
        ...
        <div className="flex items-center space-x-4">
          ...
          <button className="text-gray-300 hover:text-white" type="button" aria-label="Settings">
        ...
        <div className="flex-1 p-6">
          <div className="bg-gray-800 rounded-lg h-full flex items-center justify-center relative">
            ...
            <div className="absolute top-4 left-4 bg-black/50 rounded-lg p-3">
              <p className="text-sm font-medium">Meeting Room: {id}</p>
          ...
          <div className="space-y-3">
            {participants.map((participant) => (
              <div key={participant.id} className="flex items-center space-x-3 p-3 bg-gray-700 rounded-lg">
                ...
                <div className="flex space-x-2" aria-label="Participant status">
                  {participant.isMicOn ? (
      ...
      <div className="bg-gray-800 p-4">
        <div className="flex items-center justify-center space-x-4">
          <button
            type="button"
            onClick={toggleMic}
            className={`p-3 rounded-full ${isMicOn ? 'bg-gray-600 hover:bg-gray-700' : 'bg-red-600 hover:bg-red-700'}`}
          ...
          <button
            type="button"
            onClick={toggleScreenShare}
            className={`p-3 rounded-full ${isScreenSharing ? 'bg-red-600 hover:bg-red-700' : 'bg-gray-600 hover:bg-gray-700'}`}
