...
export default function RankRewardsProgress() {
  ...
  const lifetimeEarned = Number(data?.lifetimeEarned || 0);
  ...
  const remaining = next ? Math.max(0, nextThreshold - lifetimeEarned) : 0;

  const progressPct = useMemo(() => {
    if (!next) return 100;
    const span = Math.max(0.000001, nextThreshold - currentThreshold);
    const raw = ((lifetimeEarned - currentThreshold) / span) * 100;
    return Math.max(0, Math.min(100, raw));
  }, [next, nextThreshold, currentThreshold, lifetimeEarned]);
      ...
      <div className="bg-white dark:bg-gray-800 rounded-2xl p-6 border border-gray-200 dark:border-gray-700 shadow-lg">
        <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-4">
          <div>
            ...
            <p className="text-3xl font-bold text-gray-900 dark:text-white">${formatMoney(lifetimeEarned)} USDT</p>
            <p className="mt-1 text-sm text-gray-600 dark:text-gray-400">
              Current tier:{' '}
            ...
            {next ? (
              <>
                ...
                <p className="text-sm text-gray-600 dark:text-gray-400">
                  Earn <span className="font-semibold text-gray-900 dark:text-white">${formatMoney(remaining)}</span> more
                </p>
              </>
            ) : (
              <>
            ...
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-blue-800/80 dark:text-blue-200/80">
                  Your progress updates automatically as you earn commissions and credits.
                </p>
              </div>
            </div>
          </div>
