...
export default function TeacherShell({
  ...
  const current = useMemo(() => getTeacherNavItem(activeTab), [activeTab]);
  const currentGroup = useMemo(() => getTeacherNavGroup(activeTab), [activeTab]);
  ...
  const quickNavItems = useMemo(() => {
    ...
    return TEACHER_NAV_GROUPS.flatMap((group) =>
      group.items
        .filter(
          (item) =>
            !query ||
  ...
  const openTab = (tab: TeacherTabId) => {
    onTabChange(tab);
        ...
        <div className="admin-modal-overlay !items-start !pt-[12vh]">
          ...
          <div role="dialog" aria-modal="true" className="admin-modal admin-modal--lg !max-w-xl !p-0">
            <div className="flex items-center gap-3 border-b border-[var(--admin-border)] px-4">
              ...
              <input
                id="teacher-quick-nav"
                autoFocus
                value={navQuery}
                onChange={(event) => setNavQuery(event.target.value)}
                placeholder="Jump to courses, students, library..."
                className="h-14 min-w-0 flex-1 bg-transparent text-sm outline-none"
                aria-label="Jump to a teacher area"
              />
              ...
              {quickNavItems.length ? (
                quickNavItems.map((item) => {
                  ...
                  return (
                    ...
                    >
                      <span className="admin-stat-card__icon admin-stat-card--emerald !h-9 !w-9 !rounded-lg">
          ...
          <div className={`admin-sidebar__brand w-full ${collapsed ? 'justify-center' : 'pr-9 lg:pr-0'}`}>
            ...
            {!collapsed ? (
              <div className="min-w-0">
                <p className="admin-sidebar__title">Teacher</p>
          ...
          <div className="admin-topbar__inner">
            <div className="flex min-w-0 flex-1 items-center gap-3">
              ...
              <div className="admin-breadcrumb min-w-0">
                ...
                <p className="admin-breadcrumb__title truncate">{current?.label || 'Teacher'}</p>
                {current?.description ? (
            ...
            <div className="admin-topbar__tools">
              <button
                ...
                title="Jump to a teacher area (Ctrl or Command + K)"
              >
