...
import { ADMIN_NAV_GROUPS, getAdminNavGroup, getAdminNavItem, type AdminTabId } from '../config/nav';
...
export default function AdminShell({
  ...
  const current = useMemo(() => getAdminNavItem(activeTab), [activeTab]);
  const currentGroup = useMemo(() => getAdminNavGroup(activeTab), [activeTab]);
  ...
  const quickNavItems = useMemo(() => {
    ...
    return ADMIN_NAV_GROUPS.flatMap((group) =>
      group.items
        .filter((item) => !query || `${item.label} ${item.description || ''} ${group.label}`.toLowerCase().includes(query))
        .map((item) => ({ ...item, group: group.label }))
    );
  }, [navQuery]);

  const openTab = (tab: AdminTabId) => {
    onTabChange(tab);
          ...
          <div role="dialog" aria-modal="true" className="admin-modal admin-modal--lg !max-w-xl !p-0">
            <div className="flex items-center gap-3 border-b border-[var(--admin-border)] px-4">
              ...
              <input
                ...
                placeholder="Jump to payments, users, settings..."
                className="h-14 min-w-0 flex-1 bg-transparent text-sm outline-none"
                aria-label="Jump to an admin area"
              />
              ...
              {quickNavItems.length ? (
                quickNavItems.map((item) => {
                  ...
                  return (
                    ...
                    >
                      <span className="admin-stat-card__icon admin-stat-card--indigo !h-9 !w-9 !rounded-lg">
                      ...
                      <span className="text-[10px] font-semibold uppercase tracking-wide text-slate-400">{item.group}</span>
                    </button>
                  );
                })
              ) : (
          ...
          <div className="admin-topbar__inner">
            <div className="flex min-w-0 flex-1 items-center gap-3">
              <button
                ...
                className="admin-icon-btn border border-[var(--admin-border)] lg:hidden"
                aria-label="Open navigation"
              >
                <Menu className="h-5 w-5" />
              </button>
              <div className="admin-breadcrumb min-w-0">
                ...
                <p className="admin-breadcrumb__title truncate">{current?.label || 'Admin'}</p>
                {current?.description ? (
            ...
            <div className="admin-topbar__tools">
              <button
                ...
                title="Jump to an admin area (Ctrl or Command + K)"
              >
              ...
              <div className="admin-topbar__tool-group">
                ...
                <button
                  ...
                  title="Settings"
                >
                  <SettingsIcon className="h-[1.125rem] w-[1.125rem]" />
                </button>
              </div>
